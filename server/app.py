...
- 如果 improvements 涉及口癖/语气词过多,suggestion 里必须给出 1-2 个可直接套用的替代表达(例如"然后"→"接下来/与此同时","那个"→"这一点"或直接停顿),不要只说"减少口癖"。
- 如果 improvements 涉及结构松散,suggestion 可以对症建议一个具体框架(如"这部分试试空雨伞:先说事实,再给分析,最后落到行动"或"用过去—现在—未来串一下");只在确实对症时用,最多一处,不要每条建议都硬塞框架。
...
CATEGORY_RUBRICS = {
    "观点": "本题是观点表达题。观点维度评判:立场是否明确、一致、出现不过晚;并结合受众栏判断内容是否对接听众想听的(开口前先想清楚\"听众想听什么\")。结构维度评判:有无清晰框架(可借用\"过去—现在—未来\"\"空雨伞:事实—分析—行动\"\"3C\"等成熟框架),段落间有无\"路标式\"衔接(首先/接下来/最后)。内容维度评判:是否有理由+例子+个人判断;重点能否一句话说清(核心句);是否做了减法(表达者的职责是筛选而非倾倒);例子是否具象(有行为有场景,而非形容词堆砌)。",
    "解释": "本题是概念解释题。观点维度评判:核心定义/结论是否尽早给出(\"这是什么\"),不评判立场;内容维度看是否通俗、有无类比和例子、有无术语堆砌;结构维度看\"是什么→为什么/怎么用→总结\"。",
    "工作": "本题是工作汇报/介绍题(含产品介绍)。观点维度评判:结论/核心信息是否先行,不评判立场;并结合受众栏判断内容是否对接听众最关心的(如对领导讲结果与风险、对同事讲配合)。结构维度评判:是否按\"背景→问题→措施→结果\"或\"空雨伞:事实—分析—行动\"一类清晰框架组织,段落间有无\"路标式\"衔接。内容维度评判:是否有具体事实、数据、措施;重点能否一句话说清(核心句);是否做了减法(筛选该汇报什么,而非倾倒全部)。",
    "日常": "本题是日常话题题。观点维度评判:是否有明确的个人答案,不评判立场;内容是否对接受众栏里听众想听的。结构维度评判:叙述有无清晰顺序(可借用\"过去—现在—未来\"),段落间有无\"路标式\"衔接。内容维度评判:是否有具体经历和细节;重点能否一句话说清;是否具象(有行为有场景,而非形容词堆砌)。",
    "公考面试": (
...
def build_user_prompt(payload: dict) -> str:
    ...
    rubric = CATEGORY_RUBRICS.get(
        category,
        "按通用口头表达标准评判:核心信息明确(重点能否一句话说清)、结构清晰(有框架,段落间有\"首先/接下来/最后\"式路标衔接)、内容具体(有行为有场景,做了筛选而非倾倒)、表达流畅;并结合受众栏判断内容是否对接听众想听的。",
    )
    lines = [
...
def call_prep_hints(payload: dict) -> dict:
    ...
    if category == "公考面试":
        ...
        user += (
            "思考提示按这条准备主线组织提问:我为什么讲(动机、我在意什么)→听众想听什么→"
            "我用什么框架(如\"过去—现在—未来\"\"空雨伞:事实—分析—行动\"\"3C\")→我的核心句是什么(一句话说清重点);"
            "3-4 条提示尽量依次对应这几个环节,仍是提问式,不给答案。\n"
        )
